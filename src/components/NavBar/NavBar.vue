<template>
  <view class="nav" :style="navBackground">
    <view class="status" :style="'height: ' + statusHeight + 'rpx;'"></view>
    <view v-if="props.isHome" class="headNavbar" :style="'height: ' + navHeight + 'rpx;' + 'line-height: ' + navHeight + 'rpx;' + 'padding-left: 20rpx;'"> 
      <text class="city">中部地区</text>
      <view style="flex: 1;">
        <navigator url="/subPages/search/search" class="search-box">
         <text class="search-text">搜索医院名称</text>
        </navigator>
      </view>
    </view>
    <view class="navbar" :style="'height: ' + navHeight + 'rpx;'" v-else>
      ...
      <view class="title" :style="navFont">{{ props.title }}</view>
    </view>
  </view>
</template>
...
const navFont = ref('')
const setNavSize = () => {
  const {statusBarHeight = 0, system} = uni.getSystemInfoSync()
...
// 设置样式
const setStyle = () => {
  navBackground.value = `background: ${props.background};`
  navFont.value = `color: ${props.color}; font-size: ${props.fontSize}rpx;`
}
